...
PROCESS_ID_FILE=$APP_BASE/fightserver.pid
...
case "$1" in
    start)
        ...
        $JAVA_HOME/bin/java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=7778 -Xmx512m -Xms512m -Xmn384m  -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath="./OutOfMemory.hprof"  -Dcom.sun.management.jmxremote.port=7801 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -Djava.rmi.server.hostname=192.168.1.20 -cp "$PATH" "$APP_NAME" "$CONFIG_FILE" &
        echo $! > $PROCESS_ID_FILE
